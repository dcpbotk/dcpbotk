...
export const data = {
  name: "mrol-menu-rolkaldir",
  description: "Kullanıcıdan rol kaldır.",
  ...
  async execute(interaction) {
    ...
    try {
      await member.roles.remove(values)
      response.setDescription(`\`✅\`<@${member.id}>, ${values.map(roleId => `<@&${roleId}>`)} rolleri başarıyla kaldırıldı.`)
      await interaction.editReply({ embeds: [response], ephemeral: true });
    } catch (error) {
      response.setDescription(`\`❌\`<@${member.id}>, roller kaldırılırken hata oluştu.\n\`❕\`Seçilen rollerden en az birini eklemeye veya kaldırmaya iznim yok.`)
      await interaction.editReply({ embeds: [response], ephemeral: true });
    }
